...
| **버전** | 1.5 |
| **작성일** | 2026-02-12 |
| **최종 수정** | 2026-02-13 — P0–P4 완료, Optimistic Update, 문서 정합성 갱신 |
...
| 훅 | queryKey (queryKeys.ts) | 데이터 소스 | staleTime |
|----|-------------------------|-------------|-----------|
| `useVehicles` | `vehicleKeys.list(ownerId, status)` | Mock | 5분 |
| `useVehicle` | `vehicleKeys.detail(vehicleId)` | Mock | 5분 |
| `useAuction` | `auctionKeys.detail(vehicleId)` | Mock | 0 (5초 폴링) |
| `useInspections` | `inspectionKeys.list(...)` | Mock | (기본) |
| `useSettlements` | `settlementKeys.list(filter)` | Mock | 5분 |
| `useSettlement` | `settlementKeys.detail(id)` | Mock | 5분 |
| `useLogisticsSchedule` | `logisticsKeys.schedule()` | Mock | 5분 |
| `useLogisticsHistory` | `logisticsKeys.history()` | Mock | 5분 |
| `useSalesHistory` | `saleKeys.history()` | Mock | 5분 |
| `useVehicleRegister` | mutation | Mock | — |
| `useInspectionRequest` | mutation | apiClient | — |
| `useBid` | mutation (Optimistic Update) | apiClient | — |
| `useBuyNow` | mutation (Optimistic Update) | apiClient | — |
...
| 페이지 | 훅 | 데이터 소스 | 비고 |
|--------|-----|-------------|------|
| VehicleListPage | useVehicles | Mock | ✅ |
| TradeListPage | useVehicles | Mock | ✅ (차량 기반 필터) |
| VehicleDetailPage | useVehicle | Mock | ✅ |
| AuctionDetailPage | useAuction | Mock | ✅ (5초 폴링) |
| InspectionListPage | useInspections | Mock | ✅ |
| InspectionRequestStep1 | useVehicles | Mock | ✅ |
| SettlementListPage | useSettlements | Mock | ✅ |
| SettlementDetailPage | useSettlement | Mock | ✅ |
| LogisticsSchedulePage | useLogisticsSchedule | Mock | ✅ |
| LogisticsHistoryPage | useLogisticsHistory | Mock | ✅ |
| SalesHistoryPage | useSalesHistory | Mock | ✅ |
...
| 페이지 | 비고 |
|--------|------|
| **GeneralSaleOffersPage** | 제안 수락/거절 mutation. useProposals 미구현 |
| **DashboardPage** | 요약 데이터 (추정) |

### 3.3 Mutation invalidation 현황

| mutation | invalidation 대상 | Optimistic |
|----------|-------------------|------------|
| useVehicleRegister | `vehicleKeys.all` | — |
| useInspectionRequest | `inspectionKeys.all`, `vehicleKeys.all` | — |
| useBid | `auctionKeys.auctions` | ✅ 취소·스냅샷·롤백 |
| useBuyNow | `auctionKeys.auctions`, `vehicleKeys.all` | ✅ 취소·스냅샷·롤백 |
...
AuthProvider의 `user` 객체도 `useUser` 쿼리로 관리하는 것을 고려. 백엔드 Auth API의 인증 상태를 Query의 subscription으로 연결하면, **모든 상태를 QueryClient 하나로 통합 관리** 가능해 디버깅과 동기화가 쉬워짐.
...
**참고**: apiEndpoints.ts에 GET 정산/탁송/매출 목록·상세 API가 없으면 Mock 폴백으로 먼저 구현. 백엔드 확장 시 ERD_Mapping·api_v1 §4 반영.
...
| 구분 | 현시점 | 목표 | 비고 |
|------|--------|------|------|
| **Query 적용 범위** | vehicles, inspections, auction, 정산·탁송·판매 | ✅ 완료 | P0 |
| **Mutation invalidation** | vehicleKeys, inspectionKeys, auctionKeys 적용 | ✅ 완료 | P0 |
| **경매 실시간성** | useAuction refetchInterval 5000 | ✅ 완료 | P1 |
| **Optimistic Update** | useBid, useBuyNow 취소·스냅샷·롤백 | ✅ 완료 | 제안 수락/거절은 추후 |
| **Key Factory & Zod** | queryKeys.ts + schema.parse | ✅ 완료 | P2 |
| **Infinite Queries** | 단순 useQuery | 제외 | P3 |
| **Selective Persist** | whitelist(vehicles, settlements) | ✅ 완료 | P4 |
| **Auth 확장** | boolean | User 객체 또는 useUser | 추후 |
| **Prefetching** | 미적용 | 라우터 hover 시 선로딩 | 추후 |
...
| **데이터 소스** | Mock + apiClient | useVehicles/useInspections/useVehicleRegister는 Mock. apiClient는 별도(mockFallback) | ✅ 문서와 일치 |
...
| **Mock vs API** | useVehicles/useInspections는 Mock. apiClient는 백엔드 API. API 응답 시 Zod parse가 필수. |
...
| 단계 | 항목 | 내용 | 산출물 |
|------|------|------|--------|
| **Step 1** | auctionKeys.detail 타입 | `vehicleId: string \| undefined` 허용 | queryKeys.ts |
| **Step 2** | Optimistic Update | useBid, useBuyNow에 onMutate·onError 롤백 | place-bid/model/useBid.ts, useBuyNow.ts |
| **Step 3** | 문서 정합성 | §1.2, §3.1, §3.2, §3.3, §10 갱신 | STATE_MANAGEMENT_POLICY.md |
...
| **2.3** | API 훅 import 일관성 | features 전부 `@/shared/api/client` | ✅ useBid, useBuyNow, useInspectionRequest, ocrApi — `apiClient` from `@/shared/api/client`. useVehicleRegister는 Mock 전용 (API 미사용) |
