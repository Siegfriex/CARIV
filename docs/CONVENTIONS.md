...
### 3.1 Mock vs apiClient

| 도메인 | 소스 | 비고 |
|--------|------|------|
| 차량 목록 | Mock | `useVehicles`: `MOCK_VEHICLES_ALL` |
| 경매 입찰·즉시구매 | apiClient | 백엔드 API → mockFallback |
| 검차 신청·결과 | apiClient | 백엔드 API → mockFallback |
| 탁송 | apiClient | 백엔드 API → mockFallback |
| 정산 | Mock (인라인) | 백엔드 API 연동 전 |
| 일반 판매 | Mock (인라인) | 백엔드 API 연동 전 |
| 검차 목록 | Mock (mockInspectionList) | 백엔드 API 연동 전 |
